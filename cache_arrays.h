...
class CacheArray : public GlobAlloc {
    public:
        ...
        virtual int32_t lookup(const Address lineAddr, const MemReq* req, bool updateReplacement,uint32_t& secondSearch) = 0;

        /* Runs replacement scheme, returns tag ID of new pos and address of line to write back*/
        virtual uint32_t preinsert(const Address lineAddr, const MemReq* req, Address* wbLineAddr, Address* firstAddr, uint32_t& candidate_2,bool& sec) = 0;
...
class SetAssocArray : public CacheArray {
    ...
    public:
        ...
        int32_t lookup(const Address lineAddr, const MemReq* req, bool updateReplacement, uint32_t& secondSearch);
        uint32_t preinsert(const Address lineAddr, const MemReq* req, Address* wbLineAddr, Address* firstAddr ,uint32_t& candidate_2,bool& sec);
        void postinsert(const Address lineAddr, const MemReq* req, uint32_t candidate);
};

//******************************************************//
/*EBSC style cache array*/
class EBSCArray : public CacheArray {
    protected:
        Address* array;
        Address* array_reuse;
        ...
        //******************************//
        // per cache line
        uint32_t* d_bit; // identify this cache line is second map or not

        // per set 
        // there should be changed by 
        // store the source set;
        int source_num;
        ...
        // hash function for two different mapping
        HashFamily* hf_1;
        HashFamily* hf_1_c;
    ...
    public:
        ...
        EBSCArray(uint32_t _numLines, uint32_t _assoc, ReplPolicy* _rp, HashFamily* _hf_1,HashFamily* _hf_1_c, HashFamily* _hf_2, HashFamily* _hf_2_c);

        //**************// 
        int getRemapId(int sset, int *set_list, uint32_t &tset);
        int allocSpace(int sset, int *set_list, uint32_t &tset);
        //**************//
        void updateStat(uint32_t set);
        int32_t lookup(const Address lineAddr, const MemReq* req, bool updateReplacement, uint32_t& secondSearch);
        uint32_t preinsert(const Address lineAddr, const MemReq* req, Address* wbLineAddr, Address* firstAddr,uint32_t& candidate_2,bool& sec);
        void postinsert(const Address lineAddr, const MemReq* req, uint32_t candidate);
