...
class CC : public GlobAlloc {
    public:
        ...
        virtual void moveSingleAddr(Address repl_addr,int32_t repl_line,Address origin_addr, int32_t origin_line){;};
...
class MESICC : public CC {
    ...
    public:
            ...
            if ((req.type == GETS) || (req.type == GETX)) {
                ...
                if (!nonInclusiveHack) {
                    panic("[%s] We lost inclusion on this line! 0x%lx, type %s, childId %d, childState %s, lineId is %x", name.c_str(),
                            req.lineAddr, AccessTypeName(req.type), req.childId, MESIStateName(*req.state), array->lookup(req.lineAddr,NULL,false,secondSearch));
                }
                return false;
            }
        }
        ...
        void moveSingleAddr(Address repl_addr,int32_t repl_line,Address origin_addr, int32_t origin_line){
            // repl_addr : target address for this move option
            //
            tcc->evictAddr(repl_addr, repl_line);
            bcc->setInvalid(repl_addr, repl_line);
            tcc->moveAddr(repl_addr, repl_line, origin_addr, origin_line);
            bcc->moveAddr(repl_addr, repl_line, origin_addr , origin_line);
            array->moveAddr(repl_addr, repl_line, origin_addr , origin_line);
        }
