...
int Cache::checkSkew(MemReq& req, int *raceDetected) {
   uint32_t secondSearch = 0;
   int lineId = array->lookup(req.lineAddr,&req,false,secondSearch);
   return lineId;
}
//Check the skew and find out whether cache line is in there
int Cache::checkSkewN(MemReq& req, int *raceDetected) {
    if(req.lineAddr == 986125){
        std::cout << " check 986125 " << std::endl;
    }
   uint32_t secondSearch = 0;
   int lineId = array->lookup(req.lineAddr,&req,false,secondSearch);
   return lineId;
...
uint64_t Cache::accessSkew(MemReq& req) {
    ...
    if (likely(!skipAccess)) {
        ...
        uint32_t secondSearch = 0;
        uint32_t candidate_2 = 0;
        bool     sec = false;

        int32_t lineId = array->lookup(req.lineAddr, &req, updateReplacement,secondSearch);
        respCycle += accLat;
        ...
        if (lineId == -1 && cc->shouldAllocate(req)) {
            ...
            //std::cout << name.c_str() << " access " << req.lineAddr << std::endl;
            lineId = array->preinsert(req.lineAddr, &req, &wbLineAddr, &firstAddr, candidate_2,sec); //find the lineId to replace
            //std::cout << "lineId = " << lineId << " SecondId = " << candidate_2 << std::endl;
            ...
            // int32_t lineIdsel = (sec == false) ? lineId : candidate_2;
            // cc->processEviction(req, wbLineAddr, lineIdsel, respCycle); 
            ...
            //if(!sec){
            //    std::cout << "Evicting " << wbLineAddr << std::endl;
            //    cc->processEviction(req, wbLineAddr, lineId, respCycle); //1. if needed, send invalidates/downgrades to lower level
            //}else{
            //    std::cout << "Moving " << firstAddr << " to " << wbLineAddr << std::endl;
            //    cc->moveSingleAddr(wbLineAddr , candidate_2 , firstAddr , lineId);
            //}
            ...
            if(!sec){
               ...
               // std::cout << "Moving " << firstAddr << " to " << wbLineAddr << std::endl;
                //std::cout << "**********" << std::endl;
                // cc->processEviction(req, wbLineAddr, candidate_2, respCycle);
                cc->moveSingleAddr(wbLineAddr , candidate_2 , firstAddr , lineId);
            }
...
uint64_t Cache::access(MemReq& req) {
    ...
    if (likely(!skipAccess)) {
        ...
        // this means when it need an second search, in pipeline, it only need 1 cycle to get this value
        if(secondSearch == 0)
            respCycle += accLat;
        else
            respCycle += (accLat + 1);
...
uint64_t Cache::finishInvalidate(const InvReq& req) {
    uint32_t secondSearch = 0;
    int32_t lineId = array->lookup(req.lineAddr, nullptr, false,secondSearch);
...
bool Cache::finishRefresh(Address lineAddr, uint32_t lineId){
   uint32_t secondSearch = 0;
  // std::cout << "refresh the l1 cache" << lineAddr << std::endl;
   int32_t lineId_new = array->lookup(lineAddr, nullptr, false,secondSearch);
   assert(lineId_new != -1);
